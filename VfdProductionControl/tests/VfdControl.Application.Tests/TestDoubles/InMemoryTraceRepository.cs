...
public sealed class InMemoryTraceRepository : ITraceRepository
{
    ...
    public List<StepRunSnapshot> Steps { get; } = [];
    ...
    public Task<IReadOnlyList<StationSessionSummary>> QuerySessionsAsync(TraceabilitySessionQuery query, CancellationToken ct)
    {
        var queryResults = Sessions
            ...
            .Select(session =>
            {
                var runs = DeviceRuns.Where(run => run.SessionId == session.SessionId).ToList();
                return new StationSessionSummary(
                    session.SessionId,
                    session.StationId,
                    session.OperatorCode,
                    session.StartedAt,
                    AggregateConclusion(runs),
                    runs.Count);
            })
            .ToList();

        return Task.FromResult<IReadOnlyList<StationSessionSummary>>(queryResults);
    }

    public Task<IReadOnlyList<DeviceRunSummary>> QueryDeviceRunsAsync(DeviceRunQuery query, CancellationToken ct)
    {
        var queryResults = DeviceRuns
        ...
        return Task.FromResult<IReadOnlyList<DeviceRunSummary>>(queryResults);
    }
